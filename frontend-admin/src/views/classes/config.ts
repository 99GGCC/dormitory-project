...
export function useConfig(API: {
  ...
  const gridOptions = reactive<VxeGridProps<RowVO>>({
    ...
    formConfig: {
      items: [
        ...
        createVxeFormItem('select', {
          field: 'classesStatus', title: '班级状态', span: 4,
          options: useDictStore().getDict('CLASSES_STATUS'),
        }),
        createVxeFormItem('input', {
          field: 'instructorName', title: '辅导员姓名', span: 4,
        }),
        createVxeFormItem('input', {
          field: 'instructorPhone', title: '联系电话', span: 4,
        }),
        {
          itemRender: {
            name: '$button',
    ...
    columns: [
      ...
      { field: 'classesStatus', title: '班级状态', formatter(params) {
        return params.row.classesStatus === 0 ? '正常' : '已毕业'
      } },
      { field: 'instructorName', title: '辅导员姓名' },
      { field: 'instructorPhone', title: '联系电话' },
  ...
  const formItems = ref([
    ...
    createVxeFormItem('select', {
      field: 'classesStatus', title: '班级状态',
      options: useDictStore().getDict('CLASSES_STATUS'),
    }),
    {
      field: 'majorId', title: '专业', span: 24,
      slots: {
  ...
  const formRules = ref({
    classesName: [createRequiredValidateRule()],
    instructorName: [createRequiredValidateRule()],
    instructorPhone: [createRequiredValidateRule(), createRegExpValidateRule(isPhoneNumber)],
